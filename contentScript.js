...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  ...
  if (request.correct_answer) {
    ...
    var questionElement = document.querySelector(`a[name="${request.question_id}"]`).parentElement;
    if (questionElement) {
      ...
      answerInputs.forEach(element => {
        ...
        if (answers.includes(answer_text)) {
          ...
          element.querySelector(".answer_label").style.border = "2px solid green";
          element.querySelector(".answer_label").style.padding = "1%";
          element.querySelector(".answer_label").classList.add("extension-visible");
        }
      });
